...
def LSIG_demodulator(samples,samplingRate,LSTF_endIndex,LLTF_channel,LSIG_symbol):
    ...
    try:
        ...
        for i in pilot64.index:
            phaseShift = phaseShift + (temp[i]/LLTF_channel[i])/pilot64.symbol[pilot64.index.index(i)];    
        ...
        #-----------------------------------------------------------------------------------------------------------
        #RL-SIG detection
        startIndex2 = LSTF_endIndex + int(samplingRate*(LLTF_length + L_cp + LSIG_length)) + 1;
        temp = [];
        ...
        if abs(correlation)/power > 0.9:
            ...
            #MRC
            for i in range(len(symbol)):
                symbol[i]= (symbol[i]/abs(symbol[i]) + symbol2[i]/abs(symbol2[i]))/2;
...
def LSIG_decoder(LSIG_symbol,LSIG_bits,LSIG_info):
    ...
    try:
        ...
        LSIG_info["Rate"] = str(LSIG_bits[0]) + str(LSIG_bits[1]) + str(LSIG_bits[2]) + str(LSIG_bits[3]);
